...
namespace FiendMagicDestiny_bot
{
    internal class StateMachine 
    {
        ...
        private FileDestiny file = new FileDestiny(fileName);
        private FileDestiny file2 = new FileDestiny(fileName2);
        public StateMachine()
        {
        ...
        {
            //fileName2 = $"ДОПОЛНЕНИЕ: {StateMachine._Name[chatId]}_Предназначение.docx"; //в работе
            //file2.CreateDocument();
            string data = $"\r\n\r\n\r\n\r\n\r\n\r\n    Дополнение: \r\n {_Addition[chatId]}";
            //WriteAddition(data);

            await file.SendingFile(botClient, chatId, fileName);
            //await file2.SendingFile(botClient, chatId, fileName2);
            file.DeleteDocument();
            //file2.DeleteDocument(); 
        }
        public void BuilderList(long chatId)
        {

            fileName = $"{StateMachine._Name[chatId]}_Предназначение.docx";
            file.CreateDocument(fileName);
            ...
            foreach (short obj in Arcs)
            {
                if (repeats.ContainsKey(obj))
                {
                    ...
                    if (Arcans.ContainsKey(obj) && !addedArcans.Contains(obj))
                    {
                        ...
                        WriteData(data);
                        file.FormattingTextToBold(boldWords);
                        addedArcans.Add(obj);
...
&& (repeats[obj] > 1 || repeats[obj2] > 1 || repeats[obj3] > 1 || (repeats[obj] == 1 && obj4 != obj && obj3 != obj2 && obj4 != obj2)) && repeats.ContainsKey(obj4))
        ...
        public void WriteInstructions(string fileName, long chatId)
        {
            ...
            file.WriteToFile(instructions);
            
        }
        public void WriteAddition(string data)
        {
            Console.WriteLine(data);
            file2.WriteToFile(data);
        }
